...
# ==========================================
    ...
    # --- RESERVAS ---
        ...
        if user['perfil'] == "Síndico":
                    ...
                    if r['status'] == "Aprovada": st.success(f"📅 {r['data_reserva']} | APROVADA - Churrasqueira liberada!")
                        ...
                        if st.button("Confirmar Pagamento", key=f"btn_comp_{r['id']}"):
                            if arq_comp:
                                ...
                                # --- NOTIFICA SÍNDICO DO COMPROVANTE ---
                                email_admin = st.secrets["email"]["endereco"]
                                msg_admin = f"O morador da Casa {user['casa']} enviou o comprovante de pagamento para a reserva do dia {r['data_reserva']}.\n\nAcesse o painel para aprovar a reserva."
                                enviar_notificacao(email_admin, f"💵 Comprovante de Reserva: Casa {user['casa']}", msg_admin)
            ...
            data_escolhida = st.date_input("Selecione a data no calendário:", value=None, key=f"calendario_{st.session_state['chave_calendario']}")
            ...
            if data_escolhida:
                ...
                if status_data == "Aprovada": st.error("⚠️ Data já reservada.")
                    ...
                    if st.button("Solicitar Data"):
                        ...
                        # --- NOTIFICA SÍNDICO DA NOVA SOLICITAÇÃO ---
                        email_admin = st.secrets["email"]["endereco"]
                        msg_admin = f"O morador da Casa {user['casa']} solicitou a reserva para {data_str}.\nAcesse o painel para anexar a cobrança."
                        enviar_notificacao(email_admin, f"📅 Nova Reserva: Casa {user['casa']}", msg_admin)
                        
                        st.session_state['chave_calendario'] += 1 # Reseta o calendário
                        st.success("Solicitação enviada com sucesso! Aguarde a liberação da cobrança.")
                        st.rerun()
            ...
            with st.expander("📂 Novo Balancete"):
                with st.form("form_bal", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Salvar") and arq_bal:
                        ...
                        executar_sql("INSERT INTO balancetes (titulo, nome_arquivo, arquivo_dados) VALUES (?, ?, ?)", (titulo, arq_bal.name, arq_bal.getvalue()))
                        st.rerun()
            st.divider()
        ...
        if user['perfil'] == "Síndico":
            with st.expander("➕ Aplicar Multa"):
                ...
                with st.form("form_multa", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Aplicar") and infrator and motivo and arq_multa:
                        ...
                        executar_sql("INSERT INTO multas (casa, motivo, data_aplicacao, nome_arquivo, arquivo_dados) VALUES (?, ?, ?, ?, ?)", (num_casa, motivo, hoje, arq_multa.name, arq_multa.getvalue()))
                        
                        # --- NOTIFICA MORADOR ---
                        email_infrator = buscar_dados("SELECT email FROM usuarios WHERE casa=? LIMIT 1", (num_casa,))
            ...
            for m in buscar_dados("SELECT * FROM multas ORDER BY id DESC"):
                ...
                if status_multa == 'Em Análise':
                    ...
                    if c3.button("✅ Dar Baixa", key=f"dbm_{m['id']}"):
                        executar_sql("UPDATE multas SET status='Paga' WHERE id=?", (m['id'],))
                        st.rerun()
                
                if c4.button("🗑️ Excluir", key=f"xxm_{m['id']}"):
            ...
            for m in minhas_multas:
                ...
                if status_m == 'Pendente':
                    ...
                    if m.get('arquivo_dados'):
                        st.download_button("📄 Baixar Notificação/Boleto", data=bytes(m['arquivo_dados']), file_name=m['nome_arquivo'], key=f"dmm_{m['id']}")
                    ...
                    if st.button("Confirmar Pagamento", key=f"bcm_{m['id']}") and arq_comp:
                        executar_sql("UPDATE multas SET status='Em Análise', comprovante_nome=?, comprovante_dados=? WHERE id=?", (arq_comp.name, arq_comp.getvalue(), m['id']))
                        
                        # --- NOTIFICA SÍNDICO DO COMPROVANTE ---
                        email_admin = st.secrets["email"]["endereco"]
                        msg_admin = f"O morador da Casa {user['casa']} enviou o comprovante de pagamento referente à multa: '{m['motivo']}'.\n\nAcesse o painel para dar baixa na multa."
                        enviar_notificacao(email_admin, f"💵 Comprovante de Multa: Casa {user['casa']}", msg_admin)
